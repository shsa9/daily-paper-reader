...
tldr: 本文针对具有时变延迟和外部扰动的广义Persidskii系统，提出了一种结合稳定性分析与数据驱动状态估计的综合框架。研究通过Lyapunov-Krasovskii泛函推导出输入状态稳定性（ISS）的线性矩阵不等式（LMI）条件，并设计了鲁棒观测器。利用保稳定性的Koopman提升技术从轨迹数据中识别系统参数，并将其应用于ICODE-MPPI控制器。在1.5 kW永磁同步电机上的实验验证了该方法在提升估计精度和跟踪性能方面的显著优势。
selection_source: fresh_fetch
motivation: 旨在解决具有非线性反馈和时变延迟的复杂机电系统在存在模型不确定性时的稳定性分析与高精度状态估计难题。
method: 开发了基于LMI的ISS稳定性条件，并结合保稳定性的Koopman提升算法进行数据驱动建模，同时设计了$H_\infty$同步准则下的鲁棒观测器。
result: "在永磁同步电机实验中，该方法相比扩展卡尔曼滤波降低了35%的速度估计误差，并使速度跟踪精度较标准矢量控制提升了67%。"
conclusion: 研究证明了广义Persidskii框架在处理非线性时延系统建模与控制中的有效性，为高性能电机驱动提供了理论支撑与实践指导。
---

## 摘要
本文研究了受时变时滞和外部扰动影响的广义 Persidskii 系统的稳定性分析和状态估计问题。广义 Persidskii 类系统将线性动力学与扇区受限的非线性反馈回路相结合，为机电系统和神经网络系统的建模提供了一个易于处理且具有表现力的框架。我们通过结合 Persidskii 型积分项的 Lyapunov-Krasovskii 泛函，开发了输入状态稳定性 (ISS) 的时滞相关条件，并将这些条件转化为线性矩阵不等式 (LMI)。针对具有部分状态测量的系统，提出了一种结构化鲁棒观测器，并通过 H∞ 同步准则保证了其收敛性。为了处理受控对象的不确定性，利用保持稳定性的 Koopman 提升程序从轨迹数据中识别系统矩阵，其中 ISS-LMI 约束在参数回归过程中作为凸侧条件嵌入。识别出的模型填充了 ICODE-MPPI（输入相关控制导向动力学估计——模型预测路径积分）控制器的预测时域。整个框架在配备可编程负载制动器的 1.5 kW 永磁同步电机 (PMSM) 驱动器上进行了验证。实验结果证实，与扩展卡尔曼滤波相比，速度估计的均方根误差 (RMSE) 降低了 35%，与标准场定向控制相比，转速跟踪精度提高了 67%，从而验证了本文建立的理论 ISS 边界。
