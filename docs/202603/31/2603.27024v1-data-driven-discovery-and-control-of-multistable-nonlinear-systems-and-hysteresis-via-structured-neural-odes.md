...
tldr: 针对具有多稳态和滞后特性的非线性系统，本文提出了一种结构化的神经常微分方程（Neural ODE）架构。该方法通过将向量场分解为收缩项和平衡点映射项，在确保轨迹稳定性的同时实现了对多吸引子位置的参数化。实验证明，该架构在短时间序列数据上训练高效，能准确捕捉复杂的吸引盆分布，并支持基于隐式平衡点映射的高效梯度反馈控制。
selection_source: fresh_fetch
motivation: 旨在解决非线性多稳态系统在数据驱动建模中存在的稳定性难以保证、模型非唯一性以及控制困难等挑战。
method: 设计了一种特殊的Neural ODE结构，通过构造包含收缩项和平衡点映射项的向量场，强制执行轨迹稳定性并简化多稳态参数化。
result: 在多个非线性基准测试中，该模型在短时域训练下表现高效，成功捕捉了多个吸引盆，并实现了基于梯度的闭环反馈控制。
conclusion: 这种结构化建模方法为多稳态非线性系统的系统辨识与控制提供了一种兼具物理一致性和计算效率的新方案。
---

## 摘要
许多工程物理过程表现出非线性但渐近稳定的动力学特性，这些特性收敛于由控制输入决定的有限平衡点集。从数据中识别此类系统具有挑战性：稳定的动力学提供的激励有限，且模型发现往往是不唯一的。我们提出了一种极简结构化的神经常微分方程（NODE）架构，该架构通过学习形式为 $F(x,u) = f(x)\,(x - g(x,u))$ 的向量场，强制执行轨迹稳定性并为多稳态系统提供可处理的参数化。其中，$f(x) < 0$（逐元素）确保了收缩性，而 $g(x,u)$ 决定了多吸引子的位置。在多个非线性基准测试中，所提出的结构在短时间跨度训练上表现高效，能够捕捉多个吸引盆，并通过隐式平衡映射 $g$ 实现高效的基于梯度的反馈控制。
